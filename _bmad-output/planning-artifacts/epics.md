...
用户可以配置模型 provider、API key、base URL、custom endpoint 和 model name，并对页面图片执行多模态分析；系统生成符合 `page_analysis_v1` schema 的页面 JSON，失败时可定位、可重试；当模型识别不精准时，用户可以通过纠错提示词重新生成 JSON，或在全屏编辑器中直接修正 JSON，并确保密钥不泄露到日志、错误提示或导出文件。

**FRs covered:** FR5, FR6, FR17, FR22, FR23

**实现备注:** 包含模型 provider/endpoint 抽象、keyring 密钥保存、API key redaction、分析任务编排、schema 校验、`analysis_results` 入库、页面级 JSON 生成、失败重试、纠错提示词重新生成、手动 JSON 编辑、原子保存和分析结果来源审计。启用云端模型前必须让用户知道页面图片会发往配置的模型服务。
...
### Story 3.8: 全屏 JSON 编辑器与统一可信结果保存管线

**FRs implemented:** FR23

As a 本地文档处理用户,  
I want 可以全屏查看、校验并手动保存页面 JSON,  
So that 当模型输出不准确时，我能直接修正页面元数据，并让修正结果以可信、可追溯、可检索的方式成为当前有效 JSON。

**Acceptance Criteria:**

**Given** 用户正在查看某个页面的当前有效 `page_analysis_v1` JSON  
**When** 用户点击“全屏编辑 JSON”入口  
**Then** 应打开全屏或近全屏 JSON 编辑界面，默认载入当前有效 JSON 和 `base_analysis_result_id`  
**And** 编辑器应提供保存、取消、格式化、校验和关闭入口，并保留当前页面图片预览或清晰返回路径

**Given** 用户在全屏编辑器中修改 JSON  
**When** 用户关闭、取消、切换页面或导航离开  
**Then** 系统不得隐式写入修改  
**And** 如果存在未保存草稿，应提示用户保存、放弃或取消离开

**Given** 用户输入了非法 JSON  
**When** 用户点击校验或保存  
**Then** 系统应显示清晰的中文语法错误、行列位置或 JSON path 提示  
**And** 不得写入 SQLite、JSONL、索引或当前有效分析结果

**Given** 用户输入了合法 JSON 但不符合 `page_analysis_v1` schema  
**When** 用户点击校验或保存  
**Then** 系统应显示字段级或路径级 schema 错误，包括缺失字段、类型错误、schema version 错误或敏感字段错误  
**And** 不得保存为当前有效 JSON

**Given** 用户输入的 JSON 通过语法校验、`page_analysis_v1` schema 校验和敏感信息检查  
**When** 用户保存  
**Then** 前端应调用 service-layer command 保存手动编辑结果，不得直接写 SQLite、JSONL、索引文件或页面 artifact  
**And** 保存输入应至少包含 `document_id`、`page_id`、`base_analysis_result_id`、编辑后的 JSON 和 `source_type = "manual_edit"`

**Given** 保存命令进入后端 service 层  
**When** 后端保存手动编辑结果  
**Then** 必须通过统一保存管线，例如 `analysis_result_writer.save_validated_result(input)`，执行 schema validation、version creation、atomic write、current pointer switch、JSONL sync 和 index update/stale marking  
**And** Story 3.8 和后续提示词重生成故事都必须复用该保存管线，不得复制持久化逻辑

**Given** 当前有效结果在用户打开编辑器后已被其他流程改变  
**When** 用户基于旧 `base_analysis_result_id` 保存  
**Then** service 应返回 `conflict` 结构化错误，要求用户重新加载或基于新版本再保存  
**And** 不得覆盖新的当前有效结果

**Given** 手动编辑结果保存成功  
**When** 系统记录分析结果元数据  
**Then** 必须创建新的 analysis result version，并保留旧结果  
**And** 记录应包含 `source_type = "manual_edit"`、`base_analysis_result_id`、`created_at`、关联 `page_id`、schema version 和可选审计摘要

**Given** 手动编辑结果通过校验且 artifact 持久化成功  
**When** 系统提交保存  
**Then** `current_analysis_result_id` 或等价当前有效结果指针才可以切换到新版本  
**And** 如果 SQLite 写入、page-level artifact 写入或指针切换任一步失败，上一版有效 JSON 必须保持可用

**Given** 手动保存成功  
**When** 页面 JSON/JSONL artifact 和搜索索引状态需要同步  
**Then** 应从 SQLite 权威账本重新生成或更新该页页面 JSON/JSONL artifact  
**And** 应局部更新该页 BM25 索引；如果 MVP 暂不支持局部更新，必须将索引标记为 `stale` 或 `needs_rebuild` 并让搜索页可见

**Given** artifact 写入、JSONL 同步或索引更新失败  
**When** 保存流程结束  
**Then** 系统必须按固定策略回滚当前指针或标记 artifact/index stale，不得沉默失败  
**And** 用户应看到中文错误摘要、可恢复建议和 correlation_id 或诊断入口

**Given** 手动保存流程涉及 API key、token、原始模型响应或错误详情  
**When** 写入日志、错误、JSONL、页面 JSON、搜索响应或前端状态  
**Then** API key、token、完整原始模型响应和未脱敏错误详情不得出现  
**And** 手动编辑审计信息不得泄露 secret

**Given** 用户在常见桌面窗口尺寸下打开全屏 JSON 编辑器  
**When** 查看编辑区、图片预览、工具栏、错误提示和保存入口  
**Then** 文本、按钮、错误提示和编辑内容不得互相遮挡  
**And** 对常见 `page_analysis_v1` JSON 长度，输入、滚动、格式化、校验和保存不得让 GUI 卡死

**Given** 开发者为本故事编写测试  
**When** 覆盖核心保存路径  
**Then** 测试至少应覆盖 invalid JSON、schema-invalid JSON、valid manual edit、cancel/close 无写入、conflict save 不覆盖 current result、persist failure 无半写 artifact、stale index path 可观察  
**And** 所有测试不应依赖真实外部模型服务或真实 API key

### Story 3.9: 基于用户纠错提示词重新生成单页 JSON

**FRs implemented:** FR22

As a 本地文档处理用户,  
I want 在发现页面 JSON 内容不准确时输入纠错提示词让模型重新生成该页 JSON,  
So that 我可以把例如“这张图女性为女星宋茜”这样的人工判断反馈给模型，并得到更准确、仍然可信的结构化元数据。

**Acceptance Criteria:**

**Given** 用户正在查看某个已分析页面的图片预览和当前 `page_analysis_v1` JSON  
**When** 用户打开“纠错提示词重生成”入口  
**Then** 界面应显示可输入纠错提示词的编辑区域，并保留页面图片、当前标题/摘要和当前 JSON 的上下文  
**And** 入口可以出现在搜索页 JSON 查看区或工作台页面详情中，但必须调用同一后端 service

**Given** 用户输入纠错提示词，例如“这张图女性为女星宋茜”  
**When** 用户提交重新生成  
**Then** 前端应调用 Tauri command 或 service client 创建单页重新分析 job  
**And** 请求应包含 `document_id`、`page_id`、`base_analysis_result_id`、用户提示词和必要的上下文引用，不得由前端直接调用模型 endpoint

**Given** 后端收到单页纠错重生成请求  
**When** `AnalysisService` 组装模型请求  
**Then** prompt 应包含原页面图片、当前有效页面 JSON、用户纠错提示词和 `page_analysis_v1` 输出约束  
**And** 模型 provider、prompt template、schema validator、analysis repository 和 job orchestrator 边界必须与普通分析流程一致

**Given** 模型返回候选 JSON  
**When** schema validator 校验结果  
**Then** 只有通过 JSON 语法校验、`page_analysis_v1` schema 校验和敏感信息检查的候选结果才能进入确认/保存流程  
**And** 校验失败时不得持久化为 current result，不得移动 current-valid-result pointer

**Given** 候选 JSON 校验通过  
**When** 前端展示重新生成结果  
**Then** 界面应让用户明确知道这是候选结果，并提供确认保存或放弃入口  
**And** 应尽量展示变更摘要或 before/after 差异，帮助用户确认模型是否按纠错提示修正

**Given** 用户确认保存候选 JSON  
**When** 保存命令进入后端 service 层  
**Then** 必须复用 Story 3.8 建立的统一保存管线，例如 `analysis_result_writer.save_validated_result(input)`  
**And** 保存输入应包含 `source_type = "model_regeneration"` 或等价来源、`base_analysis_result_id`、correction prompt 安全摘要、provider、model name 和关联 job ID

**Given** 当前有效结果在用户发起重生成后已被其他流程改变  
**When** 用户基于旧 `base_analysis_result_id` 确认保存候选 JSON  
**Then** service 应返回 `conflict` 结构化错误，要求用户重新加载或基于新版本再次重生成  
**And** 不得覆盖新的当前有效结果

**Given** 用户确认保存且结果通过统一保存管线  
**When** 系统提交保存  
**Then** 必须创建新的 analysis result version，保留旧结果，并将当前有效结果指针切换到新版本  
**And** 记录应包含 `source_type = "model_regeneration"`、`base_analysis_result_id`、`created_at`、模型标识、提示词安全摘要和关联 `page_id`

**Given** 保存成功  
**When** 页面 JSON/JSONL artifact 和搜索索引状态需要同步  
**Then** 应从 SQLite 重新生成或更新该页 JSON/JSONL artifact  
**And** 应局部更新该页 BM25 索引；如果 MVP 暂不支持局部更新，必须将索引标记为 `stale` 或 `needs_rebuild` 并在 GUI/API 中暴露该状态

**Given** 模型调用失败、返回非法 JSON、schema 校验失败、用户放弃候选结果或保存失败  
**When** 重新生成流程结束  
**Then** 旧的当前有效 JSON 必须保持可用  
**And** 用户应看到中文失败原因、可重试状态和 correlation_id 或诊断入口

**Given** 用户连续多次对同一页面执行纠错重生成  
**When** 多个 job 并发或先后完成  
**Then** 系统应通过 `base_analysis_result_id`、当前有效结果指针、更新时间或 job 顺序避免旧结果覆盖新结果  
**And** job events 只作为 UI 提示，不得成为当前 JSON 的 source of truth

**Given** 重新生成流程涉及 API key、token、原始模型响应或用户提示词  
**When** 写入日志、错误、JSONL、页面 JSON、搜索响应或前端状态  
**Then** API key、token 和完整原始模型响应不得出现  
**And** 用户提示词如需保存，应只保存非敏感摘要或审计字段，并避免泄露到搜索结果中

**Given** localhost API 或 Tauri command 调用本流程  
**When** 出现校验、冲突、模型调用、持久化或索引状态问题  
**Then** 应返回统一结构化错误，例如 `schema_invalid`、`conflict`、`model_failed`、`persist_failed`、`index_stale`  
**And** 错误响应不得包含 secret、完整原始模型响应或未脱敏提示词

**Given** 开发者为本故事编写测试  
**When** 覆盖核心重生成路径  
**Then** 测试至少应覆盖 schema invalid 不移动 current pointer、model failure 保持旧结果、conflict 不覆盖新 current result、successful regeneration 创建新 version/audit/JSONL/index 状态、persist failure 无半写 artifact  
**And** 所有测试应使用 mock provider，不依赖真实外部模型服务或真实 API key

### Epic 4: 本地 BM25 索引与搜索体验

用户可以基于页面分析结果构建或重建本地 BM25 索引，在搜索页用中文关键词检索页面，看到相关性排序、分数、来源文档、页面图片预览和页面 JSON；搜索页 JSON 查看区提供进入纠错重生成和全屏编辑的入口，索引不可用、正在重建或因 JSON 修正变为 stale 时界面有明确状态提示。
...
**FRs implemented:** FR9, FR15, FR22, FR23
...
**When** 用户在 JSON 查看区选择纠错提示词重生成或全屏编辑 JSON  
**Then** 搜索页应打开对应入口，并保留当前搜索结果和页面选择上下文  
**And** 重新生成和保存逻辑必须调用 Epic 3 的 shared services，不得由搜索页直接写入模型、SQLite、JSONL 或索引文件

**Given** 页面 JSON 被纠错重生成或手动编辑后保存成功  
**When** 搜索页刷新该结果详情  
**Then** 应显示最新当前有效 JSON  
**And** 如果索引被标记为 stale 或 needs_rebuild，应显示清晰的刷新/重建提示，不得静默展示过期检索状态
